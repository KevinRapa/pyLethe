...
class RoomGraph(object):
    ADJACENTS = {}   # Maps IDs to a list of adjacent rooms.
    COORDINATES = {} # Maps IDs to coordinates.
    INT_TO_ID = {}   # Maps coordinates to IDs.
    ...
    def getIds():
        ids = ( 
            NULL,ENDG,HADS,
            # FLOOR 1 ROOM ID'S ---------------------------------------------- 
            COU1,COU2,COU3,COU4,COU5,COU6,COU7,VEST,FOY1,FOY2,FOYW,FOYB,FOYC,
            GAL1,GAL2,LIB1,LIB2,LIB3,BHA1,BHA2,BHA3,OBS1,STUD,PAR1,LOOK,ROTU,
            MHA1,CHS1,SQUA,SHA2,IHA1,MHA2,DIN1,SHAR,SHA1,IHA2,MHA3,KITC,WBAL,
            WOW1,WOW2,COUS,DST1,EOW1,EOW2,COU8,FOR1,FOR2,FOR3,FOR4,FOR5,
            # FLOOR 2 ROOM ID'S ---------------------------------------------- 
            GAL5,LIB4,OBS2,JHA1,PAR2,FOY3,GAL3,GAL4,LIB5,SST1,JHA2,GAR1,GAR2,
            DIN2,GAR3,GAR4,DRAR,WOW3,CLOS,WORK,EOW4, 
            # FLOOR 3 ROOM ID'S ---------------------------------------------- 
            OBS3,ATT1,LABO,FOY4,GAL6,GAL7,SST2,ATT2,BLS1,TOW1,CHS3,CHA1,CHA2, 
            # FLOOR 4 ROOM ID'S ----------------------------------------------
            SOUL,TBAL,BLS2,TOW2,LQU1,LQU2,CEL1,CEL2,CEL3,CEL4,CEL5,CEL6,
            # BASEMENT ROOM ID'S --------------------------------------------- 
            CAS1,CRY2,VAU1,CIS2,CIS1,SEW5,PRIS,TORC,CRY1,VAU2,CIS4,AARC,SEW4,
            SEW3,SEW2,SEW1,VAUE,CIS3,OUB1,INTR,SEWP,DKCH,SEW0,ESC1,ESC2,ESC3,
            ESC4,ESC5,ESC6,CIS5,
            # CATACOMBS ROOM ID'S -------------------------------------------- 
            CS35,OU62,CT11,CT12,CT13,CT14,CT15,TM16,CT17,MY18,CT21,CT22,CT23,
            CT24,CT25,CT26,CT27,CT28,CT31,TM32,CT33,CT34,CT36,CT37,CT38,CT41,
            CT42,CT43,CT44,CT45,CT46,CT47,CT48,CT51,CT52,CT53,CT54,AN55,CT56,
            CT57,CT58,CT61,CT63,CT64,AN65,TM66,CT67,CT68, 
            # CAVES ROOM ID'S ------------------------------------------------ 
            CV11,CV12,CV13,CV14,CV15,CV16,CV17,CV18,CV21,CV22,CV23,CV24,CV25,
            CV26,CV27,CV28,CV31,CV32,CV33,CV34,CV35,CV36,CV37,CV38,CV41,CV42,
            CV43,CV44,CV45,CV46,CV47,CV48,CV51,CV52,CV53,CV54,CV55,CV56,CV57,
            CV58,CV61,CV62,CV63,CV64,MS65,MS66,CV67,CV68 
        )
    ...
    def constructRoomGraph():
        ...
        ids = RoomGraph.getIds()
        
        for key in ids: 
            RoomGraph.ADJACENTS[key] = adjs[index]
            index += 1  

        RoomGraph._assignCoordinates(ids); 
    ...
    def _assignCoordinates(ids): 
        ...
        for i in range(0, len(ids)):
            c = coords[i];
            baseTen = (c[0] * 100) + (c[1] * 10) + c[2]
            RoomGraph.INT_TO_ID[baseTen] = ids[i]
            RoomGraph.COORDINATES[ids[i]] = coords[i]


    # Bootstrap method for when a game begins.
    @staticmethod
    def assignCoordinates():
        RoomGraph._assignCoordinates(RoomGraph.getIds());
    ...
    @staticmethod
    def getCoords(ID):
        return RoomGraph.COORDINATES[ID]


    """
        For any set of coordinates, this returns the room ID at those coordinates.
        @param c Z, Y, and X coordinates
        @return The ID of the room with those coordinates.
    """
    @staticmethod
    def getRoomID(coords):
        # Converts the array to a base 10 number to use as an index.
        baseTen = (coords[0] * 100) + (coords[1] * 10) + coords[2]
        
        if baseTen in RoomGraph.INT_TO_ID:
            return RoomGraph.INT_TO_ID[baseTen]
        else:
            return NULL